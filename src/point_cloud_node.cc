...
int main(int argc, char **argv)
{
	...
	float fps = cam.getRawFrameRate();
	cv::Size frame_size = cam.getRawFrameSize();

	
	int IPLastSegment = 33;
	std::array<int, 5> udpPORT = std::array<int, 5>{9201, 9202, 9203, 9204, 9205};
	std::string pipeline = "appsrc ! video/x-raw,format=BGR ! queue ! videoconvert ! video/x-raw,format=I420 ! nvvidconv ! omxh264enc ! video/x-h264,stream-format=byte-stream ! h264parse ! rtph264pay"; 
	pipeline = pipeline
		+ " ! udpsink host=192.168.123."
	       	+ std::to_string(IPLastSegment)
	        + " port="
		+ std::to_string(udpPORT[camPosNum-1]);
	...
	cv::VideoWriter output(
			...
			fourcc,
			fps,
			cv::Size(frame_size.width >> 2, frame_size.height >> 1),
			1);
