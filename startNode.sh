...
if [ "${patchlines}" == "1" ]; then
    ...
    if [ "${camNum1}" == "2" ]; then
        ...
        rosrun unitree_camera_utils pub_image _run_config_file:=${SCRIPT_DIR}/config/stereo_camera_config.yaml camera${camNum1} &
    else
    ...
    if [ "${camNum2}" == "2" ]; then
        ...
        rosrun unitree_camera_utils pub_image _run_config_file:=${SCRIPT_DIR}/config/stereo_camera_config.yaml camera${camNum2} &
    else
